...
class User_DB{
    ...
    **/
    ...
    public function viewAll($arr_values){
        ...
        $sql = "  SELECT concat('user_', @rownum:=@rownum+1) AS DT_RowId, qr_user.*,qr_role.ROLE_NAME FROM (SELECT @rownum:=0) r, qr_user, qr_role where qr_user.role_id = qr_role.ROLE_ID and (LOGIN_NAME like '%". $search_con ."%' or USER_NAME like '%". $search_con ."%' or USER_TEL like '%". $search_con ."%') ";
